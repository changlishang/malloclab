...
static block_t* explicit_listp = NULL;
...
void *malloc(size_t size) 
{
    ...
    {  
        ...
        if (block == NULL) // free returns an error
        {
            return bp;
        }
        if (explicit_listp == NULL) {
            explicit_listp = block;
            explicit_listp -> prev = NULL;
            explicit_listp -> next = NULL;
        }
    }
...
static block_t *find_fit(size_t asize)
{
    ...
    for (block = explicit_listp; block != NULL;
                             block = block -> next)
    {
...
static void write_footer(block_t *block, size_t size, bool alloc)
{
    word_t *footerp = (word_t *)(block -> payload - 16 + get_size(block) - dsize);
...
static block_t *payload_to_header(void *bp)
{
    return (block_t *)(((char *)bp) - offsetof(block_t, prev));
}
...
bool mm_checkheap(int lineno) {
    ...
    while (get_size(heap_checker_head) != 0) {
        ...
        if (heap_checker_head != heap_listp && (extract_size(*prev_footer) != get_size(heap_checker_head) || extract_alloc(*prev_footer) != get_alloc(heap_checker_head))) {
...
bool mm_checklist(int lineno) {
    block_t* block;
    for (block = explicit_listp; block != NULL; block = block -> next) {
        if (!in_heap(((void *)block))) {
            dbg_printf("Explicit list pointer not in heap in %d\n", lineno);
                return false;
        }
        if (block -> next != NULL && !in_heap((void *)(block -> next))) {
            dbg_printf("Explicit list next pointer not in heap in %d\n", lineno);
                return false;
        }
        if (block == explicit_listp) {
            if (block -> prev != NULL) {
                dbg_printf("Explicit list header's prev pointer is not NULL in %d\n", lineno);
                return false;
            }
        }
        else {
            if (block -> prev -> next != block) {
                dbg_printf("Explicit list prev and next not matched in %d\n", lineno);
                return false;
            }
        }
        
    }
    dbg_printf("Correct checklist in %d\n", lineno);
    return true;
}